...
const EmployeeFormPage = ({employees}) => {
  ...
  const handleSaveEmployee = async (employeeData) => {
    ...
    try {
      if (employee) {
        await axios.put(`https://localhost:7162/api/Employees/${employee.id}`, requestData);
        console.log("Edited successfully");
        navigate('/myEmployees');
      } else {
        await axios.post("https://localhost:7162/api/Employees", requestData);
